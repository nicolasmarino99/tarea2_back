...
# Find the existing user with the username 'testl'
user = User.find_by(username: 'testl')

# Ensure the user exists before attempting to create products
if user
  ...
  # Create products and associate them with the user
  products.each do |product_data|
    user.products.create!(product_data)
  end

  puts "Seeded #{products.size} products for user #{user.username}."
else
  puts "User with username 'testl' not found. No products were seeded."
end
